...
| 风险管理 | 部分实现 | 登记、评估、措施状态机与历史、关闭前措施门禁、统一活动时间线、逾期摘要与列表排序可用；评分规则无管理页面 |
...
- 支持风险编号/描述/Owner/Program 搜索，以及功能线、状态、等级、Study、逾期筛选和服务端分页。
- 快捷卡展示总数、Open、高风险、中风险；统计由服务端返回。
- 列表支持按更新时间、编号、Study、评分、等级、登记日期排序；措施列展示未完成数与逾期数。
- 新建风险先选择 Study，Program、Project 由 Study 关系只读带出。
...
- 风险可有多条控制措施（后续跟踪）；措施状态机为 OPEN→IN_PROGRESS→COMPLETED/CANCELLED，允许 OPEN→COMPLETED、OPEN→CANCELLED；终态需经重新打开回到 IN_PROGRESS。
- COMPLETED 必须填写实际完成日期与完成说明；CANCELLED 必须填写取消说明；计划完成日期必填。
- 存在 OPEN/IN_PROGRESS 措施时禁止关闭风险；关闭后措施只读，仅允许重开后再维护。
- 详情提供统一活动时间线（评估、开闭、措施变更）；开闭与措施变更写入专用历史表。
...
- Flyway 正式迁移链已移除演示管线（原 MySQL V14）与演示里程碑（原 MySQL V15）种子；新空库不再自动写入演示业务数据。
...
| D-13 | P0 | 已关闭 | 已移除 MySQL Flyway V14/V15 演示种子；已应用环境需单独清理 `flyway_schema_history` 与演示数据 |
