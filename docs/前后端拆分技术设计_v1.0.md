...
| `GET /programs`、`GET /projects` | 管线配置实体选择 | 按编码/名称查询真实实体，用于“存在则更新、不存在则创建” |
...
| `POST /programs` | `config.create` | 新建 Program；名称未传时自动使用 Program 编号 |
| `POST /programs/{id}/rename-impact` | `config.update` | 预览 Program 重命名影响的 Project/Study 数量并返回版本时间 |
| `PATCH /programs/{id}` | `config.update` | 修改 Program；业务编码不可修改，重命名必须带预览确认 |
| `DELETE /programs/{id}` | `config.delete` | 有 Project/Study 引用时返回 409 |
| `POST /projects` | `config.create` | 新建 Project 并关联 Program；名称未传时自动使用 Project 编号 |
| `POST /projects/{id}/rename-impact` | `config.update` | 预览 Project 重命名影响的 Study 数量并返回版本时间 |
| `PATCH /projects/{id}` | `config.update` | 修改Project名称、适应症、TA等；Project状态不落库，重命名必须确认 |
| `DELETE /projects/{id}` | `config.delete` | 有 Study 引用时返回 409 |
| `POST /studies` | `config.create` | 按 `projectId` 新建 Study；校验 Study No. 唯一 |
| `PATCH /studies/{id}` | `config.update` | 修改 Study 名称、所属 Project 和 Phase Status |
| `DELETE /studies/{id}` | `config.delete` | 有团队、里程碑、月报或风险明细时返回 409 |

重命名只更新当前 Program/Project 主数据，不批量改写既有 Study 快照；本期不建设改名专用审计、合并工具，也不修改既有排序规则。
