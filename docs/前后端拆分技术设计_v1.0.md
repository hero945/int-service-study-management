...
| `PATCH /programs/{id}` | `config.update` | 修改 Program；业务编码不可修改 |
| `POST /projects` | `config.create` | 新建 Project 并关联 Program |
| `PATCH /projects/{id}` | `config.update` | 修改Project名称、适应症、TA等；Project状态不落库 |
...
[MySQL建表语句](database/hd_plt_full_schema.sql)为唯一依据，共25张表。
...
- 当前25表脚本已在本机MySQL 8.0.46空库执行成功，但尚未替换试验性的Flyway `V1__baseline.sql`；Java运行时适配完成前不得把旧MVP验证描述为25表验证。
