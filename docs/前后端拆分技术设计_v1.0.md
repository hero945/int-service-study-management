...
本系统建议从“浏览器内单体原型”拆为“平台前端壳 + 临床研发管线业务 Tab + Java 模块化单体后端 + MySQL + 对象存储”的结构。当前 HTML 不是未来平台本身，而是平台中的一个业务 Tab。第一期后端不拆微服务、前端不直接上微前端：先建立清晰模块契约和路由级懒加载，在团队所有权或独立发布需求出现后再演进，避免过早引入分布式复杂度。
...
- 所有写操作使用稳定 ID、服务端校验、事务、乐观锁和审计；不再按姓名推导权限，不再保存明文密码，不再把业务数据放在 `localStorage`。
...
| `coverpage-msedit-v2` | `Study|阶段-节点 -> 日期/备注` | 不迁移；新数据写入 `study_milestones`、`milestone_plan_versions` |
...
| 层 | 建议 | 理由 |
|---|---|---|
| 前端平台 | React + TypeScript + Vite | 平台壳、Tab 注册、路由级懒加载和共享设计系统 |
...
| 数据访问 | Spring Data JPA 或 MyBatis（二选一并统一） | 禁止一个模块内混用多套持久层风格；复杂读模型可使用显式 SQL |
...
| 文件 | S3 兼容对象存储 | 保存异步导出文件，不把大文件放数据库 |
| 部署 | 平台前端静态站点 + Java 单服务 + MySQL + 对象存储 | 第一阶段统一发布，保留按模块拆分能力 |
...
export interface PlatformTabModule {
  ...
  title: '临床研发管线';
  route: '/platform/clinical-pipeline/*';
...
| 导出 | 提交任务、轮询、下载 | 权限快照、生成、存储、过期和审计 |
| 并发冲突 | 提示用户刷新/合并 | `version` 乐观锁，返回 409 |
...
- 主键使用 UUID/ULID；业务编号如 Study No.、Risk No. 单独设唯一约束。
- 时间：时间点用 ISO 8601 UTC，如 `2026-07-16T08:00:00Z`；业务日期用 `YYYY-MM-DD`；月份用 `YYYY-MM`。
- 列表默认 `page=1&pageSize=20`，`pageSize` 最大 200；大规模导出走异步任务。
- `PATCH` 仅修改提交字段；写操作返回新的 `version`。
- 删除默认软删除或状态关闭；确需物理删除的管理动作必须二次确认和审计。
- 写请求支持 `Idempotency-Key`，避免网络重试造成重复风险或重复导出任务。
...
| `GET /api/v1/platform/roles` | 平台角色与权限模板 | 角色、权限码和默认数据范围 |
| `POST /api/v1/platform/users` | 新建平台用户 | 触发账号激活或临时密码流程 |
| `PATCH /api/v1/platform/users/{id}` | 修改平台用户 | 状态、职务等；账号标识不可随意变更 |
...
| `PUT /api/v1/platform/users/{id}/permission-overrides` | 用户权限覆盖 | 追加/禁用权限及数据范围 |
...
| `GET /team-matrix` | 团队矩阵读模型 | `q, studyIds, roleCodes`；返回成员、注册状态和可编辑标记 |
| `GET /pipeline-config` | 管线配置页 | 管理员查询 Program/Project/Study 配置 |
| `GET /reports/monthly/preview` | 月报预览 | `month, studyIds?`；返回与导出同口径的报告模型 |
| `GET /studies/{studyId}/milestones` | 里程碑页 | 返回阶段组、节点、计划版本、实际日期和活动状态 |
...
| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `POST /programs` | `config.create` | 新建 Program |
| `PATCH /programs/{id}` | `config.update` | 修改 Program；带 `version` |
| `POST /projects` | `config.create` | 新建 Project 并关联 Program |
| `PATCH /projects/{id}` | `config.update` | 修改 Project 状态、适应症等 |
| `POST /studies` | `config.create` | 新建 Study；校验 Study No. 唯一 |
| `PATCH /studies/{id}` | `study.update` | 修改 Study 基础字段 |
| `DELETE /studies/{id}` | `config.delete` | 仅无业务明细时允许软删除；已有明细应改为停用/归档 |
| `POST /studies/{id}/stage-overrides` | `milestone.override` | 人工覆盖阶段；必须填写原因和有效期 |

#### 里程碑

| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `PUT /studies/{id}/milestones/{milestoneId}` | `milestone.update` | 幂等保存实际日期、备注和计划版本 |
| `POST /studies/{id}/milestone-plan-versions` | `milestone.plan.create` | 新增计划版本，不覆盖历史版本 |
| `GET /studies/{id}/stage-projection` | `study.read` | 返回当前阶段、活动节点及推导证据 |
...
| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `GET /studies/{id}/monthly-reports?month=2026-07` | `monthly.read` | 查询 Study 当月各功能线内容 |
| `PUT /studies/{id}/monthly-reports/{month}/{functionCode}` | `monthly.update` | 按 Study+月份+功能线幂等新增/更新 |
| `POST /monthly-reports/{id}/submit` | `monthly.submit` | 可选：从草稿进入已提交，提交后按规则锁定 |
| `POST /monthly-reports/{id}/reopen` | `monthly.reopen` | 受控重开，要求原因 |

请求体示例：

```json
{"content":"本月新增入组 42 例……","status":"DRAFT","version":3}
```
...
| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `POST /risks` | `risk.create` + Study/功能线范围 | 服务端生成风险编号、评分和等级 |
...
服务端评分：`score = impact × likelihood × detectability`。当前原型阈值为低风险 1~12、中风险 13~36、高危 ≥37；正式上线前需业务签字确认，并保存 `risk_rule_version_id`，以便历史风险按当时规则重现。

#### 团队与平台用户引用

| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `PUT /studies/{id}/team-assignments/{roleCode}` | `team.update` | 用 `userIds[]` 替换某角色成员；事务处理 |
...
| 方法与路径 | 权限 | 说明 |
|---|---|---|
| `POST /exports` | 对应 `report.export.*` | 创建异步导出；请求体含月度、格式、Study 范围 |
| `GET /exports/{jobId}` | 创建人或管理员 | 查询 `PENDING/RUNNING/SUCCEEDED/FAILED/EXPIRED` |
| `GET /exports/{jobId}/download` | 创建人或管理员 | 返回短时效签名地址或流式下载 |
| `POST /api/v1/platform/admin/backups` | `backup.create` | 平台级生成加密备份，不含密码哈希明文输出 |
| `POST /api/v1/platform/admin/restore-jobs` | `backup.restore` | 平台级先验证和预演，再由双人审批执行恢复 |
...
erDiagram
  ...
  MILESTONE_DEFINITION ||--o{ STUDY_MILESTONE : defines
  ...
  USER ||--o{ DATA_SCOPE : limited_by
  USER ||--o{ AUDIT_LOG : performs
```

### 6.2 独立数据库建设要求

本项目建设独立 MySQL 数据库，建议逻辑库名为 `clinical_platform`（最终名称按公司规范确定）。开发、测试、生产必须使用不同实例或严格隔离的独立环境，禁止让测试连接生产，也禁止与其他业务系统共用表、root 账号或应用账号。
...
所有业务表默认包含：`id binary(16) PK`、`created_at datetime(3)`、`created_by binary(16)`、`updated_at datetime(3)`、`updated_by binary(16)`、`version int not null default 1`。ID 由 Java 应用生成 UUIDv7/ULID 并以 `binary(16)` 保存，API 仍输出标准字符串；时间统一以 UTC 写入。需要软删除的表再包含 `deleted_at/deleted_by`。所有表使用 InnoDB、`utf8mb4`，字符串比较规则必须在建库时统一，避免 Study No.、邮箱和中文名称在不同排序规则下产生不一致。

表命名建议区分所有权：平台公共表使用 `plt_` 前缀（如 `plt_users`、`plt_roles`、`plt_audit_logs`），临床研发管线表使用 `cp_` 前缀（如 `cp_studies`、`cp_risks`）。下文为提高可读性省略前缀。Java 包与数据库表的模块归属必须一致，业务 Tab 不得直接更新 `plt_` 表。

| 表 | 关键字段 | 约束/索引 |
|---|---|---|
| `therapeutic_areas` | `code, name_zh, name_en, sort_order, is_active` | `unique(code)` |
| `programs` | `program_no, product_name, source, origin, moa, ta_id, status` | `unique(program_no)`；索引 `ta_id,status` |
| `projects` | `program_id, project_no, indication, status` | `unique(program_id, project_no)`；索引 `status` |
| `studies` | `project_id, study_no, phase_status, status, last_business_update_at` | `unique(study_no)`；索引 `project_id,phase_status,status` |
| `function_lines` | `code, name_zh, sort_order, is_active` | `unique(code)` |
| `team_roles` | `code, name_zh, function_line_id, is_project_lead, can_manage_team` | `unique(code)` |

不建议继续使用原型中的 `Program|Study` 拼接键。数据库主键负责关联，业务编号用于展示和查询；Project 不得通过 Program 或 Study 文本反向猜测。

### 6.4 里程碑与阶段表

| 表 | 关键字段 | 约束/说明 |
|---|---|---|
| `milestone_groups` | `code, name, stage_code, sort_order` | 阶段组字典 |
| `milestone_definitions` | `group_id, code, name, sort_order, is_active` | `unique(group_id, code)` |
| `study_milestones` | `study_id, milestone_definition_id, actual_start, actual_end, note` | `unique(study_id, milestone_definition_id)` |
| `milestone_plan_versions` | `study_id, version_no, status, effective_at, reason` | `unique(study_id, version_no)` |
| `milestone_plan_dates` | `plan_version_id, milestone_definition_id, planned_date` | `unique(plan_version_id, milestone_definition_id)` |
| `stage_status_overrides` | `study_id, stage_code, status, reason, effective_from, effective_to` | 覆盖必须有原因；索引 `study_id,effective_to` |

阶段状态不建议物化为唯一真源。`StageProjectionService` 根据里程碑、Study 配置和有效覆盖计算读模型；如性能需要，可建立由任务刷新、可重建的 MySQL 投影表，不能让投影反向成为业务主数据。

### 6.5 团队与月报表

| 表 | 关键字段 | 约束/说明 |
|---|---|---|
| `team_assignments` | `study_id, team_role_id, user_id, valid_from, valid_to` | `unique(study_id, team_role_id, user_id)`；同一分配记录通过有效期维护，禁止姓名字符串关联 |
| `monthly_reports` | `study_id, month date, function_line_id, content, status, submitted_at, submitted_by` | `month` 固定为月首；`unique(study_id, month, function_line_id)` |

“应填功能线”由该 Study 的有效团队角色映射得到，并保存查询时点。若业务要求历史月报完成率不因后续团队调整而变化，应新增 `monthly_reporting_requirements(study_id, month, function_line_id, source_assignment_version)` 作为月度快照；这是上线前必须确认的口径。

### 6.6 风险表

| 表 | 关键字段 | 约束/说明 |
|---|---|---|
| `risks` | `risk_no, study_id, function_line_id, owner_user_id, description, status, registered_on, closed_at, close_reason` | `unique(risk_no)`；索引 `study_id,status,function_line_id` |
| `risk_rule_versions` | `version_no, low_max, medium_max, effective_from, effective_to, approved_by` | 生效区间不可重叠 |
| `risk_assessments` | `risk_id, impact, likelihood, detectability, score, severity, rule_version_id, reason` | `check(impact between 1 and 5)` 等；score 服务端生成 |
| `risk_actions` | `risk_id, action_type, description, owner_user_id, planned_date, completed_date, status` | 一条措施一行，替代 8 个扁平字段 |

风险登记页默认读取最新评估，但历史评估不可覆盖。关闭风险不等于删除风险，关闭和重开均记录原因。

### 6.7 身份、权限和数据范围

| 表 | 关键字段 | 约束/说明 |
|---|---|---|
| `users` | `login_name, email, display_name, title, password_hash, status, must_change_password, failed_login_count, locked_until, password_changed_at, security_stamp, last_login_at` | 登录名/邮箱唯一；状态为 `PENDING/ACTIVE/LOCKED/DISABLED`；API 永不返回 `password_hash` |
| `roles` | `code, name, description, status` | `unique(code)`，code 创建后不可改 |
| `permissions` | `code, module, action, description` | `unique(code)` |
| `user_roles` | `user_id, role_id, valid_from, valid_to` | 有效期支持临时授权 |
| `role_permissions` | `role_id, permission_id` | 联合主键/唯一约束 |
| `user_permission_overrides` | `user_id, permission_id, effect, valid_from, valid_to` | `effect=ALLOW/DENY`，DENY 优先 |
| `data_scopes` | `principal_type, principal_id, resource_type, resource_id, function_line_id, effect` | 支持角色/用户的 TA、Program、Project、Study、功能线范围 |
| `password_reset_tokens` | `user_id, token_hash, expires_at, used_at, created_by` | 只保存 token 哈希；一次性使用；过期或使用后不可恢复 |
| `password_history` | `user_id, password_hash, changed_at` | 若启用禁止重复密码策略，保存有限条旧哈希；不保存明文 |
| `spring_session` / `spring_session_attributes` | Spring Session JDBC 标准字段 | 服务端 Session 持久化，支持多实例、主动注销和并发会话控制 |

最终权限计算：`启用角色权限并集 ∪ 用户 ALLOW - 用户 DENY`，再与有效数据范围求交。团队分工可以作为授权申请/默认建议来源，但不能再通过“姓名匹配”即时产生安全权限。首个管理员不写死在源码或 SQL 示例中：部署时通过一次性初始化命令读取受控环境变量创建，并强制首次登录改密；创建成功后关闭初始化入口。

### 6.8 审计、导出与用户偏好

| 表 | 关键字段 | 说明 |
|---|---|---|
| `audit_logs` | `actor_user_id, action, object_type, object_id, before_json, after_json, reason, request_id, ip, user_agent, occurred_at` | 追加写；普通业务账号不可修改/删除 |
| `export_jobs` | `requested_by, type, params_json, permission_snapshot_json, status, object_key, expires_at, error_code` | 下载再次鉴权；文件自动过期 |
| `project_display_orders` | `ta_id, project_id, sort_order, scope_type, owner_user_id` | 需业务确认排序是全局还是个人偏好 |
| `user_preferences` | `user_id, preference_key, value_json` | 仅存纯 UI 偏好，不存业务主数据 |

### 6.9 数据完整性与事务边界

- Program/Project/Study 新增修改在单事务内维护外键和审计。
- 月报用唯一约束保证同一 Study×月份×功能线只有一条；`PUT` 做幂等 upsert。
- 团队某角色的成员替换在一个事务中完成，并验证用户状态、角色有效性和管理权限。
- 风险新建在一个事务中写入风险、首个评估、初始措施和审计。
- 乐观锁条件为 `WHERE id=? AND version=?`；影响 0 行返回 409，不允许静默覆盖。
- 所有外键字段必须使用 ID；展示名称允许变化，历史审计保存当时快照。
...
| R-06 | 并发编辑最后写入覆盖前者 | 4 | 4 | 高 | 月报、风险、里程碑修改丢失 | 所有可编辑记录加 version；409 冲突；前端展示差异和刷新入口 |
...
| R-18 | 过早微前端导致依赖与发布复杂度失控 | 3 | 4 | 中 | 多套 React/设计系统、版本冲突、排障困难 | 第一期单仓路由模块+懒加载；满足独立团队和独立发布条件后再决策 |
...
1. **创建基础设施**：独立部署本项目 MySQL、平台前端、Java 服务、对象存储和日志/监控；创建隔离网络与最小权限数据库账号。
2. **执行数据库版本脚本**：统一使用 Flyway 创建 `plt_` 平台表和 `cp_` 管线表；生产环境禁止 Hibernate 自动改表。
3. **初始化系统字典**：权限码、系统角色、TA、功能线、团队角色、里程碑定义和风险规则版本；脚本必须幂等且有版本。
...
- 当前页面可通过 `/platform/clinical-pipeline/*` 深链接进入，刷新后仍保留二级路由和筛选 URL 状态。
...
| 4 | 团队、月报、风险读写 | CRUD、乐观锁、权限和审计通过 |
| 5 | 平台账号/角色/Scope、服务端导出 | 页面与导出权限完全一致；原账号页已上移 |
