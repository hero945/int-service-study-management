-- 月报填写权限：注册 monthly.read / monthly.create / monthly.update（H2 测试库）
-- 角色策略与 mysql V12 一致：ADMIN 全量、USER 三权限、VIEWER 仅 read。
...
SELECT r.id, p.id, 'system', 'system' FROM hd_plt_role r JOIN hd_plt_permission p ON
  p.permission_code IN ('monthly.read', 'monthly.create', 'monthly.update')
...
INSERT INTO hd_plt_role_permission(role_id, permission_id, sys_create_by, sys_update_by)
SELECT r.id, p.id, 'system', 'system' FROM hd_plt_role r JOIN hd_plt_permission p ON
  p.permission_code IN ('monthly.read')
